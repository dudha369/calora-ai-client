...
import type { OnboardingData, HeightUnit, WeightUnit } from '../interfaces/Onboarding';
...
function stepPayload(step: number, d: Partial<OnboardingData>): Partial<OnboardingData> {
  switch (step) {
    ...
    case 3:  return { height: d.height };
    case 4:  return { weight: d.weight };
    case 5:  return { goal: d.goal };
    case 6: {
      const tw = d.target_weight;
      if (tw == null) return {};
      const inKg = d.weight_unit === 'lbs'
        ? Math.round((tw / 2.205) * 10) / 10
        : tw;
...
export const OnboardingPage = () => {
  ...
  useEffect(() => { historyRef.current = stepHistory; }, [stepHistory]);
  ...
  const handleNext = useCallback(async () => {
    ...
    try {
      ...
      if (currentStep === 3 && currentData.height_unit) {
        cloudStorage.setItem(CS_HEIGHT_UNIT, currentData.height_unit).catch(() => {});
      }
      if (currentStep === 4 && currentData.weight_unit) {
        cloudStorage.setItem(CS_WEIGHT_UNIT, currentData.weight_unit).catch(() => {});
      }
  ...
  return (
    <div
      ...
      style={{ paddingTop: safeTop, paddingBottom: safeBottom, backgroundColor: theme.bg_color }}
    >
